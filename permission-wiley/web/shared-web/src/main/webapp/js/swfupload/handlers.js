...
function swfUploadPreLoad() {
	...
	var loading = function () {
		...
		if (document.getElementById("divLongLoading"))
...
function swfUploadLoaded() {
	...
	if (document.getElementById("divAlternateContent"))	
...
function uploadComplete(file) {
	if (this.getStats().files_queued === 0) {
		document.getElementById(this.customSettings.cancelButtonId).disabled = true;
		// lnagy - the context name is in here - maybe can be changed to not hardcode the context
		// smarkoff - going to redefine this function in file where it's used
		// so we can deal with the context and also load into existing dialog
		// (so this will be ignored)
		document.location.href=SWFUpload.completeURL("/permission-web/sapp/commonwork/files/view");
	}
}
