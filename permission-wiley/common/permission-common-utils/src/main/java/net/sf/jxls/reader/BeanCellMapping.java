...
/**
 * lnagy - changed this class to allow instantiation of inner objects
 * After you compile, make sure to copy it in the jxls-reader-1.0.jar
...
/*    public void populateBean(String dataString, Map beans) throws IllegalAccessException, InvocationTargetException, NoSuchMethodException, ClassNotFoundException {
	...
	public void populateBean(String dataString, Map<String, Object> beans) throws IllegalAccessException,
			InvocationTargetException, NoSuchMethodException, ClassNotFoundException
	{
		log.debug("populateBean(): enter " + propertyName);
		if (beans.containsKey(beanKey))
		{
			Object bean = beans.get(beanKey);

			// try to read the property
			// if it fails with Null property value, that means maybe it is an object and it is not
			// instantiated
			// we try in that case to call setZZZ (new ZZZ ())
			Object instance = bean;
			String[] properties = propertyName.split("\\.");
			for (String property : properties)
			{
				log.debug("populateBean(): property " + instance.getClass() + " property " + property);
				...
				{
					if (e instanceof NestedNullException || e instanceof IndexOutOfBoundsException)
					{
						PropertyDescriptor pdescriptor =
								PropertyUtils.getPropertyDescriptor(instance, property);
						if (null == pdescriptor)
						{
							log.debug("populateBean(): no propertyDescriptor for property " + property);
							return;
						}

						if (pdescriptor instanceof java.beans.IndexedPropertyDescriptor)
						{
							log.debug("populateBean(): indexedProperty");
							IndexedPropertyDescriptor idxDescriptor = (IndexedPropertyDescriptor) pdescriptor;
							Method readMethod = idxDescriptor.getIndexedReadMethod();
							if (null == readMethod)
							{
								log.debug("populateBean(): no readMethod for property " + property);
								return;
							}
							Object old = PropertyUtils.getIndexedProperty(instance, property);
							log.debug("populateBean(): Value ---\n" + old + "\n----");
							if (null == old)
							{
								Class<?> returnClass = readMethod.getReturnType();
								log.debug("populateBean(): readMethod return class " + returnClass);
								try
								{
									Object obj = returnClass.newInstance();
									PropertyUtils.setIndexedProperty(instance, property, obj);
									instance = obj;
								}
								catch (InstantiationException e1)
								{
									log.debug("populateBean(): failed to initialize class " + returnClass);
									return;
								}
							}
							else
						...
						{
							Method readMethod = PropertyUtils.getReadMethod(pdescriptor);
							if (null == readMethod)
							{
								log.debug("populateBean(): no readMethod for property " + property);
								return;
							}
							Object old = PropertyUtils.getProperty(instance, property);
							log.debug("populateBean(): Value ---\n" + old + "\n----");
							if (null == old)
							{
								Class<?> returnClass = readMethod.getReturnType();
								...
								{
									Object obj = returnClass.newInstance();
									PropertyUtils.setProperty(instance, property, obj);
									instance = obj;
								}
								catch (InstantiationException e1)
								{
